...
public class UpdaterUtil {
	 ...
	 */
	public UpdaterUtil(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
	 ...
	 */
	public UpdaterUtil(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback) {
	 ...
	 * @param announce True if the program should announce the progress of new updates in console.
	 */
	public UpdaterUtil(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback, boolean announce) {
		this.plugin = plugin;
		this.type = type;
		...
		try {
			this.url = new URL(UpdaterUtil.HOST + UpdaterUtil.QUERY + this.id);
	 ...
	 * @see UpdateResult
	 */
	public UpdaterUtil.UpdateResult getResult() {
		this.waitForThread();
		return this.result;
	}
	...
	private void downloadFile() {
		...
		try {
			...
			final byte[] data = new byte[UpdaterUtil.BYTE_SIZE];
			int count;
			if (this.announce) {
				this.plugin.getLogger().info("About to download a new update: " + this.versionName);
			}
			long downloaded = 0;
			while ((count = in.read(data, 0, UpdaterUtil.BYTE_SIZE)) != -1) {
				downloaded += count;
		...
		try {
			...
			while (e.hasMoreElements()) {
				...
				if (!entry.isDirectory()) {
					...
					int b;
					final byte[] buffer = new byte[UpdaterUtil.BYTE_SIZE];
					final FileOutputStream fos = new FileOutputStream(destinationFilePath);
					final BufferedOutputStream bos = new BufferedOutputStream(fos, UpdaterUtil.BYTE_SIZE);
					while ((b = bis.read(buffer, 0, UpdaterUtil.BYTE_SIZE)) != -1) {
			...
			this.result = UpdaterUtil.UpdateResult.FAIL_DOWNLOAD;
		} finally {
	...
	private boolean hasTag(String version) {
		for (final String string : UpdaterUtil.NO_UPDATE_TAG) {
	...
	private boolean read() {
		try {
			...
			conn.addRequestProperty("User-Agent", UpdaterUtil.USER_AGENT);
			...
			this.versionName = (String) latestUpdate.get(UpdaterUtil.TITLE_VALUE);
			this.versionLink = (String) latestUpdate.get(UpdaterUtil.LINK_VALUE);
			this.versionType = (String) latestUpdate.get(UpdaterUtil.TYPE_VALUE);
			this.versionGameVersion = (String) latestUpdate.get(UpdaterUtil.VERSION_VALUE);
	...
	public interface UpdateCallback {
		 ...
		 */
		void onFinish(UpdaterUtil updater);
	}
